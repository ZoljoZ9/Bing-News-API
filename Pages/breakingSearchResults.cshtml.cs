...
{
    public class breakingSearchResultsModel : PageModel
    {
        private readonly ILogger<breakingSearchResultsModel> _logger;
        ...
        public breakingSearchResultsModel(ILogger<breakingSearchResultsModel> logger, UserDbContext dbContext)
        {
            _logger = logger;
        ...
        public async Task OnGetAsync(int pageNumber = 1)
        {
            ...
            try
            {
                ...
                string apiUrl = $"https://api.bing.microsoft.com/v7.0/news/search?q=breaking+world+news&count={pageSize}&offset={offset}&mkt=en-US";
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    else
                    {
                        ...
                        ViewData["Message"] = "An error occurred while fetching breaking world news from the Bing News API.";
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An error occurred while fetching breaking world news from Bing News API.");
